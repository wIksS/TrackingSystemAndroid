 ...
 */
public class RemoveFromGroupClickListener implements View.OnClickListener, ICallback {
    private String userName;
    private Context context;
    private IdentitySingleton identity = IdentitySingleton.getInstance();

    public RemoveFromGroupClickListener(String userName, Context context){
    ...
    public void onClick(View v) {
        ...
        data.Type = "POST";
        data.Url = "api/Group/RemoveFromGroup?id=" + this.userName;
//        data.Body = new Pair[] {new Pair("id",this.userName)};
    ...
    public void execute(final String result) {
        ((Activity)context).runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Notifier.notify(((Activity)context),"Success!", "Removed user from group!", R.drawable.ic_error_outline_black_24dp);
            }
        });    }
